...
describe('Maker', () => {
    ...
    it('adds custom courses to the course list', () => {
        // Simulate user input
        const codeInput = screen.getByPlaceholderText('AAA000');
        fireEvent.change(codeInput, { target: { value: mockCourse.code } });

        const nameInput = screen.getByPlaceholderText('Intro to Electronics');
        fireEvent.change(nameInput, { target: { value: mockCourse.name } });

        const preqInput = screen.getByPlaceholderText('AAA123,AAA456');
